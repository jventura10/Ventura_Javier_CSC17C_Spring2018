...
class LinkedList{
    ...
    public:
        ...
        void  addBeg(const T&);  //Add data at the beginning of the list
        void  addBef(const T&, const T&);  // add before the item
        void  addAft(const T&, const T&);  // Add after the item
...
void LinkedList<T>::addBeg(const T &data) {
    ...
    front=add;                 //Point to the new beginning of the list
}
...
// Add before by value
template<class T>
void LinkedList<T>::addBef(const T &data, const T &val) {
    Link *bef = (Link *)fndBef(val);
            
    Link *add=new Link;        //Create the new link
    add->data=data;            //Add the data
    add->linkPtr = bef->linkPtr;
    bef->linkPtr=add;
}

// Add after by value
template<class T>
void LinkedList<T>::addAft(const T &data, const T &val){
    Link *aft = (Link *)fndLst(val);
            
    Link *add=new Link;        //Create the new link
    add->data=data;            //Add the data
    add->linkPtr = aft->linkPtr;
    aft->linkPtr=add;
}
